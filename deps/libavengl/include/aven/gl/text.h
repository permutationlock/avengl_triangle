...
static inline AvenGLTextFont aven_gl_text_font_init(
    ...
    int ncols = 4;
    int nrows = 8;
    int char_dim = (int)((font_height_px + 2.0f) * 2.0f);

    int success = 0;
    AvenGLTextFont font = { .height_px = font_height_px };
    ...
    assert(gl->GetError() == 0);assert(gl->GetError() == 0);
...
static inline AvenGLTextGeometry aven_gl_text_geometry_init(
    ...
    geometry.umvp_location = (GLuint)gl->GetUniformLocation(
        geometry.program,
        "uMVP"
    );
    assert(gl->GetError() == 0);
...
static inline float aven_gl_text_geometry_push(
    ...
    for (size_t i = 0; i < text.len; i += 1) {
        ...
        list_push(geometry->vertices) = (AvenGLTextVertex){
            { q.x0 * pixel_size + px, q.y0 * pixel_size + py },
            { q.s0, q.t0, },
            { color[0], color[1], color[2], color[3] },
        };
        list_push(geometry->vertices) = (AvenGLTextVertex){
            { q.x1 * pixel_size + px, q.y0 * pixel_size + py },
            { q.s1, q.t0, },
            { color[0], color[1], color[2], color[3] },
        };
        list_push(geometry->vertices) = (AvenGLTextVertex){
            { q.x1 * pixel_size + px, q.y1 * pixel_size + py },
            { q.s1, q.t1, },
            { color[0], color[1], color[2], color[3] },
        };
        list_push(geometry->vertices) = (AvenGLTextVertex){
            { q.x0 * pixel_size + px, q.y1 * pixel_size + py },
            { q.s0, q.t1, },
            { color[0], color[1], color[2], color[3] },
        };
